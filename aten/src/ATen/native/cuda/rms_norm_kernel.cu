...
__global__ void RmsNormForwardCUDAKernel(
  ...
  for (int64_t j = threadIdx.x; j < N; j += blockDim.x) {
    ...
    Y[index] = static_cast<T_ACC>(X[index]) *
            static_cast<T_ACC>(rstd[i]) * gamma_v +
        beta_v;
  }
}

struct WelfordDataRmsNorm{
  float sigma2;
  float count;
  C10_HOST_DEVICE WelfordDataRmsNorm(): sigma2(0.f), count(0.f){}
  C10_HOST_DEVICE WelfordDataRmsNorm(float sigma2, float count): sigma2(sigma2), count(count) {}
};

template<typename U> __device__
WelfordDataRmsNorm cuWelfordOnlineSum(
  const U val,
  const WelfordDataRmsNorm& curr_sum)
{
  return {curr_sum.sigma2 + val * val, curr_sum.count + 1.f};
}

__device__
WelfordDataRmsNorm cuWelfordCombine(
  const WelfordDataRmsNorm dataB,
  const WelfordDataRmsNorm dataA
) {
...
template<typename T>
__device__ WelfordDataRmsNorm compute_stats(
  ...
  float * buf
    ...
    if (blockDim.y > 1) {
      ...
      if (threadIdx.x == 0 && threadIdx.y ==0) {
        sigmabuf[0] = wd.sigma2/float(N);
      }
      __syncthreads();
      return WelfordDataRmsNorm{sigmabuf[0], 0.f};

    } else {
      return WelfordDataRmsNorm{WARP_SHFL(wd.sigma2,0)/float(N), 0.f};
    }
}
...
__device__ __inline__ void vectorized_rms_norm_kernel_impl(
  const int N,
  T_ACC eps,
  const  T* __restrict__ X,
  const  T* gamma,
  ...
  T_ACC* rstd,
  T* Y){
    extern __shared__ float s_data[]; //if we made smem WelfordDataRmsNorm type, there would be bank conflicts,
    //as one thread would have to write 3 consecutive floats
    auto i1 = blockIdx.x;
    const T * block_row = X + i1 * N;
    WelfordDataRmsNorm wd = compute_stats(block_row, N, s_data);
    ...
    for (int i = thrx; i < n_vec_to_read; i += numx) {
      ...
      if (gamma_vec != nullptr && beta_vec != nullptr) {
        ...
        #pragma unroll
        for (int ii=0; ii < vec_size; ii++){
          out.val[ii] = (rstd_val * (static_cast<T_ACC>(data.val[ii]))) + static_cast<T_ACC>(beta_vec[i].val[ii]);
        }
      } else {
        #pragma unroll
        for (int ii=0; ii < vec_size; ii++){
...
template <typename T, typename T_ACC>
__global__ void vectorized_rms_norm_kernel(
  const int N,
  T_ACC eps,
  const  T* __restrict__ X,
  const  T* gamma,
  ...
  T_ACC* rstd,
  T* Y){
    vectorized_rms_norm_kernel_impl(N, eps, X, gamma, beta, rstd, Y);
  }
...
__global__ void rms_norm_grad_input_kernel(
  ...
  const int N){
    ...
    compute_gI(dY, X, rstd, gamma, dX, N, buf);
  }
...
template <typename T, typename T_ACC>
void launch_vectorized_rms_norm_kernel(
  int N,
  int64_t M,
  T_ACC eps,
  const T* X_data,
  ...
  T_ACC* rstd_data
    ...
    vectorized_rms_norm_kernel<<<blocks, threads, nshared, stream>>>(N, eps, X_data,
    gamma_data, beta_data, rstd_data, Y_data);
    ...
    while (remaining > 0) {
      ...
      vectorized_rms_norm_kernel<<<blocks, threads, nshared, stream>>>(N, eps, X_data2,
        gamma_data, beta_data, rstd_data2, Y_data2);
      C10_CUDA_KERNEL_LAUNCH_CHECK();
...
void RmsNormKernelImplInternal(
  ...
  can_vec_X && can_vec_Y && can_vec_gamma && can_vec_beta) {
    launch_vectorized_rms_norm_kernel(static_cast<int>(N), M, eps, X_data, gamma_data, beta_data, Y_data, rstd_data);
  } else {
  cudaStream_t cuda_stream = at::cuda::getCurrentCUDAStream();
  RowwiseMomentsCUDAKernel<T, T_ACC>
      <<<M, cuda_utils::kCUDABlockReduceNumThreads, 0, cuda_stream>>>(
          N, eps, X_data, rstd_data);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  RmsNormForwardCUDAKernel<T, T_ACC><<<M, kCUDANumThreads, 0, cuda_stream>>>(
      N, X_data, rstd_data, gamma_data, beta_data, Y_data);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  }
}
...
void RmsNormKernelImpl(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        ...
        RmsNormKernelImplInternal<scalar_t, acc_t>(
            X, gamma, beta, M, N, static_cast<acc_t>(eps), Y, rstd);
      });
}
    ...
    T_ACC* part_grad_beta)
{
    ...
    cuLoadWriteStridedInputs(i1_beg,thr_load_row_off,thr_load_col_off,i2_off,row_stride,warp_buf1,warp_buf2,input,dout,i1_end,N,rstd);
    for (int i1_block = i1_beg+blockDim.y*blockDim.y;  i1_block < i1_end;  i1_block+=blockDim.y*blockDim.y) {
      cuLoadAddStridedInputs(i1_block,thr_load_row_off,thr_load_col_off,i2_off,row_stride,warp_buf1,warp_buf2,input,dout,i1_end,N,rstd);
    }
    __syncthreads();
    // inter-warp reductions
...
void RmsNormBackwardKernelImplInternal(
  ...
  if (dX_data != nullptr) {
    ...
    if (M >= 32768) {
      ...
      rms_norm_grad_input_kernel<<<blocks, num_threads(), nshared, cuda_stream>>>(dY_data,
      X_data, rstd_data, gamma_data, dX_data, N);
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    }
#else
    const dim3 blocks(M);
    ...
    if (bAlignedBuffers && bTargetDataTypes && bVectorSizeMultiple) {
      rms_norm_grad_input_kernel_vectorized<<<blocks, num_threads(), nshared, cuda_stream>>>(dY_data,
          X_data, rstd_data, gamma_data, dX_data, N);
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    } else {
      rms_norm_grad_input_kernel<<<blocks, num_threads(), nshared, cuda_stream>>>(dY_data,
...
void RmsNormBackwardKernelImpl(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&]() {
        RmsNormBackwardKernelImplInternal<scalar_t>(
            dY.contiguous(), X, rstd, gamma, M, N, dX, dgamma, dbeta);
      });
}
...
std::tuple<Tensor, Tensor, Tensor> rms_norm_cuda(
  ...
  if (M > 0) {
    RmsNormKernelImpl(*X, *gamma, *beta, M, N, eps, &Y, &rstd);
  }
...
std::tuple<Tensor, Tensor, Tensor> rms_norm_backward_cuda(
    const Tensor& dY,
    const Tensor& input,
    IntArrayRef normalized_shape,
    const Tensor& rstd,
  ...
  if (M > 0 && N > 0) {
    RmsNormBackwardKernelImpl(
        dY, *X, rstd, *gamma, M, N, &dX, &dgamma, &dbeta);
  }
  return std::make_tuple(std::move(dX), std::move(dgamma), std::move(dbeta));
}

REGISTER_DISPATCH(RmsNormKernel, &RmsNormKernelImpl)
REGISTER_DISPATCH(RmsNormBackwardKernel, &RmsNormBackwardKernelImpl)
