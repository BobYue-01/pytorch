...
void RmsNormKernelImplInternal(
  ...
  at::parallel_for(0, M, 1, [&](int64_t start, int64_t end) {
    for (const auto i : c10::irange(start, end)) {
      ...
      T* Y_ptr = Y_data + i * N;
      auto [rstd_val] = RowwiseMoments(X_ptr, N);
...
template <typename T, typename param_t,
          typename std::enable_if_t<is_reduced_floating_point_v<T>, int> = 0>
void rms_norm_kernel_mixed_type(
    const Tensor& X,
    const Tensor& gamma,
    const Tensor& beta,
    int64_t M,
  ...
  at::parallel_for(0, M, 1, [&](int64_t start, int64_t end) {
    for (const auto i : c10::irange(start, end)) {
      ...
      for (; d < N - (N % bVec::size()); d += bVec::size()) {
        ...
        fVec y_fvec0 = x_fvec0 * fVec(scale) * gamma_fvec0 + beta_fvec0;
        fVec y_fvec1 = x_fvec1 * fVec(scale) * gamma_fvec1 + beta_fvec1;
...
void RmsNormKernelImplInternal(
  ...
  if (mixed_type) {
    rms_norm_kernel_mixed_type<T, float>(X, gamma, beta, M, N, eps, Y, rstd);
  } else {
    rms_norm_kernel_mixed_type<T, T>(X, gamma, beta, M, N, eps, Y, rstd);
  }
}
...
void RmsNormKernelImpl(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(kBFloat16, kHalf, X.scalar_type(),
      "RmsNormKernelImpl", [&]() {
    RmsNormKernelImplInternal<scalar_t>(
        X, gamma, beta, M, N, eps, Y, rstd);
  });
}

template <typename T, typename T2, typename opmath_t>
void rms_norm_backward_frame(
    const T* dY_data,
    const T* X_data,
    const T2* rstd_data,
    const T2* gamma_data,
  ...
  if (!dX_null) {
    ...
    const opmath_t a = rstd_data[i];
    const opmath_t b = -ds * a * a * a * scale;
    const opmath_t c = -db * a * scale;
    // Scalar math:
    // for (const auto j : c10::irange(N)) {
    //   const T gamma_v = gamma_null ? T(1) : gamma_data[j];
...
void rms_norm_backward_frame(
  ...
  if (!dgamma_null) {
    ...
    // }
    vec::map3<T>(
        [a](fVec dgamma, fVec dy, fVec x) {
          return dgamma + dy * fVec(a) * x;
        },
        dgamma_buffer_ptr,
        dgamma_buffer_ptr,
        dY_ptr,
  ...
  if (!dX_null) {
    ...
    const float a = rstd_data[i];
    const float b = -ds * a * a * a * scale;
    const float c = -db * a * scale;
    // Scalar math:
    // for (const auto j : c10::irange(N)) {
    //   const T gamma_v = gamma_null ? T(1) : gamma_data[j];
    //   dX_ptr[j] = a * dY_ptr[j] * gamma_v + b * X_ptr[j] + c;
...
void RmsNormBackwardKernelImplInternal(
  ...
  at::parallel_for(0, M, 1, [&](int64_t start, int64_t end) {
    ...
    for (const auto i : c10::irange(start, end)) {
      rms_norm_backward_frame<T, T2, opmath_t>(dY_data, X_data, rstd_data, gamma_data, dX_data, dgamma_buffer_ptr, dbeta_buffer_ptr, scale, gamma_null, dX_null, dgamma_null, dbeta_null, N, i);
    }
  });
...
void RmsNormBackwardKernelImpl(
  ...
  if (at::isReducedFloatingType(X.scalar_type())) {
    AT_DISPATCH_REDUCED_FLOATING_TYPES(X.scalar_type(), "RmsNormBackwardKernelImpl", [&]() {
      if (gamma.scalar_type() == at::kFloat) {
        RmsNormBackwardKernelImplInternal<scalar_t, float>(
            dY.contiguous(), X, rstd, gamma, M, N, dX, dgamma, dbeta);
      } else {
        RmsNormBackwardKernelImplInternal<scalar_t, scalar_t>(
            dY.contiguous(), X, rstd, gamma, M, N, dX, dgamma, dbeta);
      }
      });
  } else {
    AT_DISPATCH_FLOATING_TYPES(X.scalar_type(), "RmsNormBackwardKernelImpl", [&]() {
      RmsNormBackwardKernelImplInternal<scalar_t, scalar_t>(
          dY.contiguous(), X, rstd, gamma, M, N, dX, dgamma, dbeta);
    });
  }
